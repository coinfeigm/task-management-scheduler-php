...
function _excelColWidth( data, col ) {
	var max = getByteLength(data.header[col]) + 4;
	var len, lineSplit, str;

	if ( data.footer && getByteLength(data.footer[col]) > max ) {
		max = getByteLength(data.footer[col]);
	}
	...
	for ( var i=0, ien=data.body.length ; i<ien ; i++ ) {
		...
		if ( str.indexOf('\n') !== -1 ) {
			...
			lineSplit.sort( function (a, b) {
				return getByteLength(b) - getByteLength(length);
			} );

			len = getByteLength(lineSplit[0]);
		}
		else {
